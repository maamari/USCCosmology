...
void FisherGAL_CLUSTER::initExperiment(int sur, string label, string Mdynname, string FRname,string FRbias, string FRPk ,double SkyCov, double zmax, double intzzz, double FRR, int der_flag)
{
  ...
  CLL_name="./data/Aresults7/CLDer_"+label+"_"+strr;
  CLL_namePS="./data/Aresults7/CLDerPS_"+label+"_"+strr;
  BZfr_name="./data/Aresults7/BZfr_"+label+"_"+strr;
  nameSave="./data/Aresults7/Fisher_"+label+"_"+strr;
  nameSavePS="./data/Aresults7/FisherPS_"+label+"_"+strr;
  FR_name=FRname;
  FR_bias=FRbias;
  FR_Pk=FRPk;
  Mdyn_name=Mdynname;
...
double FisherGAL_CLUSTER::SetdndM(double m, void * params)
{
  ...
  //Cosmology my_cosmo(my_params.wm+my_params.wr, my_params.wl, my_params.wb, h, my_params.sigma8, my_params.ns, omnu, my_params.w);
  double z =c.currentZ;
  double nn=1;
...
//double FisherGAL_CLUSTER::NofzFR(My_params_N my_params, double zzz)
{
  ...
  my_params.filename=filename;
  my_params.mdyfile=mdyfile;
  ...
  t1.params = this;
  //t1.params = &my_params;
  gsl_integration_qags(&t1,1e12,2.187e16, 0, 1e-5, 1000, ww, &result, &error); 
  ...
  if (dFlag==1)
    {
      ...
      double ML=0,MMM,MM,mmm=0,mm=Mmin,intz=intzz, intm=pow(10,6*0.05),Nm, Np, Nfr;
      double beta=0,alpha=0,sigma0=0.1,Bm0=-0.15;//Bm0=-0.15;
	...
	//if (MM> ML ) {  //here I get the value of N_ml only if M is biggewr than M_threshold=ML. Moreover, if the mass before is smaller than ML, I use ML as mass2.
	    ...
	    Nfr=NofzFR(1,c->w1,c->w0,c->omm*h*h,c->oml,c->omb*h*h,0,c->sigma8, c->nscal,zz,factor*intm,factor,Bm0,sigma0,alpha,beta);
            ...
            dNN[8]=(NofzFR(0,c->w1,c->w0,c->omm*h*h,c->oml,c->omb*h*h,0,c->sigma8, c->nscal,zz,MM,ml,Bm0,sigma0,cpAlpha,beta)-NofzFR(0,c->w1,c->w0,c->omm*h*h,c->oml,c->omb*h*h,0,c->sigma8, c->nscal,zz,MM,ml,Bm0,sigma0,cmAlpha,beta))/(2*pstep);
...
void FisherGAL_CLUSTER::reduceMatrix(string label, double FRR )
{
  ...
  double *nc, *ps;
  string outnc="./data/Aresults7/FisherNew_"+label+"_"+strr+"_inverse",
         outps="./data/Aresults7/FisherPSNew_"+label+"_"+strr+"_inverse",
         outcomb="./data/Aresults7/FisherCOMBNew_"+label+"_"+strr+"_inverse";
  ...
  if (dFlag==1)
    {
      ...
      while (zz < zmaxx) {
	//fin2 >> zz  >> BZ_fr;
	BZ0=ComBiasFR(0,c->w1,c->w0,c->omm*h*h,c->oml,c->omb*h*h,0,c->sigma8, c->nscal,Bm0,sigma0,alpha,beta,zz); 
	BZfr=ComBiasFR(1,c->w1,c->w0,c->omm*h*h,c->oml,c->omb*h*h,0,c->sigma8, c->nscal,Bm0,sigma0,alpha,beta,zz); 
	BZ1p=ComBiasFR(0,c->w1,c->w0,cpomm,c->oml,c->omb*h*h,0,c->sigma8, c->nscal,Bm0,sigma0,alpha,beta,zz);
	BZ1m=ComBiasFR(0,c->w1,c->w0,cmomm,c->oml,c->omb*h*h,0,c->sigma8, c->nscal,Bm0,sigma0,alpha,beta,zz);
	BZ2p=ComBiasFR(0,c->w1,c->w0,c->omm*h*h,cpoml,c->omb*h*h,0,c->sigma8, c->nscal,Bm0,sigma0,alpha,beta,zz);
	BZ2m=ComBiasFR(0,c->w1,c->w0,c->omm*h*h,cmoml,c->omb*h*h,0,c->sigma8, c->nscal,Bm0,sigma0,alpha,beta,zz);
	BZ3p=ComBiasFR(0,c->w1,c->w0,c->omm*h*h,c->oml,c->omb*h*h,0,cpsigma8, c->nscal,Bm0,sigma0,alpha,beta,zz);
	BZ3m=ComBiasFR(0,c->w1,c->w0,c->omm*h*h,c->oml,c->omb*h*h,0,cmsigma8, c->nscal,Bm0,sigma0,alpha,beta,zz);
	BZ4p=ComBiasFR(0,c->w1,c->w0,c->omm*h*h,c->oml,cpomb,0,c->sigma8, c->nscal,Bm0,sigma0,alpha,beta,zz);
	BZ4m=ComBiasFR(0,c->w1,c->w0,c->omm*h*h,c->oml,cmomb,0,c->sigma8, c->nscal,Bm0,sigma0,alpha,beta,zz);
	BZ5p=ComBiasFR(0,c->w1,c->w0,c->omm*h*h,c->oml,c->omb*h*h,0,c->sigma8, cpnscal,Bm0,sigma0,alpha,beta,zz);
	BZ5m=ComBiasFR(0,c->w1,c->w0,c->omm*h*h,c->oml,c->omb*h*h,0,c->sigma8, cmnscal,Bm0,sigma0,alpha,beta,zz);
	BZ6p=ComBiasFR(0,c->w1,c->w0,c->omm*h*h,c->oml,c->omb*h*h,0,c->sigma8, c->nscal,cpBm0,sigma0,alpha,beta,zz);
	BZ6m=ComBiasFR(0,c->w1,c->w0,c->omm*h*h,c->oml,c->omb*h*h,0,c->sigma8, c->nscal,cmBm0,sigma0,alpha,beta,zz);
	BZ7p=ComBiasFR(0,c->w1,c->w0,c->omm*h*h,c->oml,c->omb*h*h,0,c->sigma8, c->nscal,Bm0,cpSigma0,alpha,beta,zz);
	BZ7m=ComBiasFR(0,c->w1,c->w0,c->omm*h*h,c->oml,c->omb*h*h,0,c->sigma8, c->nscal,Bm0,cmSigma0,alpha,beta,zz);
	BZ8p=ComBiasFR(0,c->w1,c->w0,c->omm*h*h,c->oml,c->omb*h*h,0,c->sigma8, c->nscal,Bm0,sigma0,cpAlpha,beta,zz);
	BZ8m=ComBiasFR(0,c->w1,c->w0,c->omm*h*h,c->oml,c->omb*h*h,0,c->sigma8, c->nscal,Bm0,sigma0,cmAlpha,beta,zz);
	BZ9p=ComBiasFR(0,c->w1,c->w0,c->omm*h*h,c->oml,c->omb*h*h,0,c->sigma8, c->nscal,Bm0,sigma0,alpha,cpBeta,zz);
	BZ9m=ComBiasFR(0,c->w1,c->w0,c->omm*h*h,c->oml,c->omb*h*h,0,c->sigma8, c->nscal,Bm0,sigma0,alpha,cmBeta,zz);
	BZ10p=ComBiasFR(0,c->w1,cpw0,c->omm*h*h,c->oml,c->omb*h*h,0,c->sigma8, c->nscal,Bm0,sigma0,alpha,beta,zz);
	BZ10m=ComBiasFR(0,c->w1,cmw0,c->omm*h*h,c->oml,c->omb*h*h,0,c->sigma8, c->nscal,Bm0,sigma0,alpha,beta,zz);
	BZ11p=ComBiasFR(0,cpw1,c->w0,c->omm*h*h,c->oml,c->omb*h*h,0,c->sigma8, c->nscal,Bm0,sigma0,alpha,beta,zz);
	BZ11m=ComBiasFR(0,cmw1,c->w0,c->omm*h*h,c->oml,c->omb*h*h,0,c->sigma8, c->nscal,Bm0,sigma0,alpha,beta,zz);
      //while (zz < zmaxx) {
...
double  FisherGAL_CLUSTER::ComBiasFR(bool frflag,double wa, double w,double wmmmhh, double wlll, double wbbbhh,double wrrr,  double sigmaa8, double nspecc, double Bm0,double sigma0,double alpha, double beta, double zzz)
{
  double Md=0,Md0=0,ML=0, MassF=0,deltaM=pow(10,0.05), zz, nn, nn2; 
  double NNN=0,NNN2=0,xm1,BIAS;
  double Bm,sigmaMM;
  double omnu = 0.0;  
  double h=sqrt(wmmmhh/(1-wlll));
 ...
 Md=1e13;
  for(int k=0;k<100;k++){
        ...
        NNN+=MassF*deltaM*erfc(xm1)* nn2*mycosmo.biasPS(zzz, Md)*Md*(deltaM-1);     //bias weighted by the mass function
        //NNN+=MassF*deltaM*erfc(xm1)* nn2*biasPS(zzz, Md,my_params)*intzz;     //bias weighted by the mass function
        NNN2+=MassF*deltaM*erfc(xm1)*Md*(deltaM-1);
        Md *= deltaM;
        //Md0=Md;
      //fin >> Md >> zz >> nn >> nn2;
  }
