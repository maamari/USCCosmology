...
int main(int argc, char *argv[])
{
  ...
  for (double i = 0.001, counter = 1; i <= 1.; i += increment) {
      if (counter == 10) {
          increment *= 10;
          counter = 1;
      }
      ...
      index++;
      ks[index] = double(i);
      pks[index] = cosm.powerSpectrum(double(i));
      myfile << ks[index] << "," << pks[index] << endl;
  }
  myfile.close();
...
// specify parameters for reset
  ...
  ////////////////////////////////////////////////////////////////
  // FisherGAL_CLUSTER_FNL CLUSTER;
  FisherGAL_CLUSTER CLUSTER;
  //FisherGAL_CLUSTER_mnu CLUSTER;
  // specify the Cluster survey: Planck, SPT, ACT
  double skycov=0;
  string label;
  if (survey ==0) {// Planck
  ...
  // cout<<endl << "calculating the CLUSTER Fisher Matrix" << endl;

  CLUSTER.specifyParams(choiceVec,fiducial);
  CLUSTER.initExperiment(survey,label, dirbase+"/Mdyn-Planck-"+frname+".dat",dirbase+"/dndm-Planck-"+frname+".dat", dirbase+"/haloBias-Planck-"+frname+".dat", dirbase+"/Pk-Planck-"+frname+".dat", skycov , 1, 0.05 , frvalue, 1);

  //CLUSTER_FNL.specifyParams(choiceVec,fiducial);
  // CLUSTER.initExperiment(survey,label,skycov , 1,0.1 , 0.0, 1);
  // CLUSTER.initExperiment(survey,label,skycov , 1, 0.05 , 0.0, 1);

  CLUSTER.derivatives();
  // CLUSTER.fisherMatrix();
  // CLUSTER.derivatives_PS();
  // cout <<endl << "calculating the POWER SPECTRUM Fisher Matrix"<< endl;
  // CLUSTER.fisherMatrix_PS();
